...
const sortByChildren = (charArray) =>charArray.sort((firstEle,secondEle)=>{
  // Solution code here...
  if(firstEle.children.length===secondEle.children.length){
    if(firstEle.name<secondEle.name){
      return -1;
    }
  }else{
    if(firstEle.children.length<secondEle.children.length){
...
const isCapitalized = (str) => {
  // Solution code here...
  let newArr=str.match(/(\b[A-Z].*?\b(.)??)/g);
return newArr
};
