...
function transformToLis(obj){
  // Solution code here...
  let newArr= Object.keys(obj).map(ele=>`<li>${key}:${obj[key]} </li>`);
  return newArr
}
...
const count = (target, input) =>input.flat().reduce((acu, value) => (value === target ? acu + 1 : acu), 0);
  // Solution code here...
...
const totalSum = (input) =>input.flat().reduce(((acu, value) => acu + value), 0);
  // Solution code here...
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newArr = input.map((element) => element.filter((value) => value % 5 === 0 && typeof value === 'number')
...
let findShortest = (data) => {
  // Solution code here...
  let newArr = data.reduce((acc, cur) => Number(acc.height) < Number(cur.height) ? acc : cur, 0);
  return newArr.name;
};
